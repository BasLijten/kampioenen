...
function inputRecord(item: unknown, fields: string[], errorMessage: string): Record<string, unknown> {
  if (typeof item !== "object" || item === null) throw new Error(errorMessage);
  const record = item as Record<string, unknown>;
  if (fields.some((field) => typeof record[field] !== "string")) throw new Error(errorMessage);
  return record;
}
...
function sourceClubs(value: unknown): SourceClub[] {
  return arrayFromJson(value, "clubs").map((item) => {
    const record = inputRecord(item, ["id", "name"], "source clubs require string id and name fields");
    return { id: stringField(record, "id"), name: stringField(record, "name") };
  });
}

function clubEloClubs(value: unknown): ClubEloClub[] {
  return arrayFromJson(value, "clubs").map((item) => {
    const record = inputRecord(item, ["slug", "name"], "ClubElo clubs require string slug and name fields");
    return { slug: stringField(record, "slug"), name: stringField(record, "name") };
  });
}

function aliases(value: unknown): ClubEloAlias[] {
  return arrayFromJson(value, "aliases").map((item) => {
    const record = inputRecord(item, ["alias", "clubEloSlug"], "aliases require string alias and clubEloSlug fields");
    ...
    return {
      ...
      clubEloSlug: stringField(record, "clubEloSlug"),
    };
  });
}
...
function eligibleFixtures(value: unknown): EligibleFixture[] {
  return arrayFromJson(value, "fixtures").map((item) => {
    const record = inputRecord(item, ["homeSourceId", "awaySourceId"], "eligible fixtures require string homeSourceId and awaySourceId fields");
    return { homeSourceId: stringField(record, "homeSourceId"), awaySourceId: stringField(record, "awaySourceId") };
  });
}
