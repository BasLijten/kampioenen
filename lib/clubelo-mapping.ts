...
export function mergeLockedMappings(
  ...
  const locks = new Map(
    existingMappings
      ...
      .map((mapping) => [`${scopeKey(mapping.scope)}:${mapping.sourceId}`, mapping])
  );

  return generatedMappings.map((mapping) =>
    locks.get(`${scopeKey(mapping.scope)}:${mapping.sourceId}`) ?? mapping
  );
}
...
export function diffClubEloMappings(
  ...
  const key = (mapping: ClubEloMapping) => `${scopeKey(mapping.scope)}:${mapping.sourceId}`;
  const previous = new Map(previousMappings.map((mapping) => [key(mapping), mapping]));
  const current = new Map(currentMappings.map((mapping) => [key(mapping), mapping]));
  ...
  return Array.from(keys)
    ...
    .flatMap((mappingKey) => {
      ...
      if (!before) return [{ sourceId: after!.sourceId, change: "added" as const, before, after }];
      if (!after) return [{ sourceId: before.sourceId, change: "removed" as const, before, after }];
      if (JSON.stringify(before) === JSON.stringify(after)) return [];
      return [{ sourceId: after.sourceId, change: "changed" as const, before, after }];
    });
}
